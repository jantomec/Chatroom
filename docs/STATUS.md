...
message they have not read yet back into the box for editing; `/model` and `/effort` pick
per agent from a list and apply from the next turn. The screen is Claude Code's shape:
